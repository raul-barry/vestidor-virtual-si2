...
@Component({
  ...
  template: `<main class="experience-page">
    @if (mode === 'recommendations') {
      <aside class="fashion-ai"><h2>FASHION AI</h2><p>Asistente por reglas con preferencias y stock real.</p><p>{{assistantReply || '¿Qué te gustaría usar hoy?'}}</p><label>Tu consulta<input [(ngModel)]="assistantQuery" (keyup.enter)="askAssistant()" placeholder="Quiero algo casual para una cena"></label><button (click)="askAssistant()" [disabled]="assistantLoading || !assistantQuery.trim()">{{assistantLoading?'Buscando…':'Enviar'}}</button><p>{{assistantError}}</p>@for(r of assistantRows;track r.id_variante){<article><strong>{{r.nombre}}</strong><p>{{r.talla}} · {{r.color}} · Bs {{r.precio}}</p><small>{{r.motivo}}</small><div><a [routerLink]="['/catalog/product',r.id_producto]">Ver producto</a><a [routerLink]="['/experience/fitting']" [queryParams]="{producto_id:r.id_producto,variante_id:r.id_variante,talla:r.talla,color:r.color}">Probar en vestidor</a><button (click)="add(r.id_variante)">Agregar</button></div></article>}</aside>
...
export class ExperienceComponent implements OnInit, OnDestroy {
  ...
  mode = ''; rows: any[] = []; staff: any[] = []; branches: any[] = []; message = ''; size = ''; color = ''; category = ''; variantId = 0; selected: any = null; busy = false; accessory = false; bodyProfile:any=null; assistantQuery=''; assistantReply=''; assistantRows:any[]=[]; assistantLoading=false; assistantError='';
  ngOnInit(): void { this.api.get('/api/body-profile').subscribe({next:(p:any)=>this.bodyProfile=p,error:()=>this.bodyProfile=null}); this.route.data.subscribe(data => { this.mode = data['mode']; this.load(); }); }
